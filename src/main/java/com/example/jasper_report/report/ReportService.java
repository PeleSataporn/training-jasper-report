...
public class ReportService {
    ...
    public InputStreamResource report () throws IOException, JRException {
        val params = new HashMap<String, Object>();
        params.put(JRParameter.REPORT_LOCALE, new Locale("th"));

        val path = "report/jasper_report.jrxml";
        val input = new ClassPathResource(path).getInputStream();
        val loader = JRXmlLoader.load(input);
        val report = JasperCompileManager.compileReport(loader);

        return processStream(report, dataReport(), params);
    }

    public JasperReportReqDto dataReport() {
    ...
    public InputStreamResource processStream (JasperReport report, Object objects, HashMap<String, Object> params) throws JRException, IOException {
        val output = new ByteArrayOutputStream();
        try (output) {
            val print = JasperFillManager.fillReport(report, params, new JRBeanCollectionDataSource(List.of(objects)));
